...
// CREATE Order
router.post("/orders", authenticateToken, async (req, res) => {
  const { CId, ODate, rows, regionSelections = [], months = [], notes = "" } = req.body;

  try {
    const [orderResult] = await db.query(
      "INSERT INTO orders (CID, ODATE, USERID, NOTES) VALUES (?, ?, ?, ?)",
      [CId, ODate, req.user.id, notes]
    );
    const OId = orderResult.insertId;
...
// UPDATE Order
router.put("/orders/:orderId", authenticateToken, async (req, res) => {
  const { OId, CId, ODate, rows, months, regionSelections, notes = "" } = req.body;

  try {
    await db.query("UPDATE orders SET CID = ?, ODATE = ?, NOTES = ? WHERE OID = ?", [CId, ODate, notes, OId]);
    await db.query("DELETE FROM order_row WHERE OID = ?", [OId]);
...
// FETCH Single Order with Regions
router.get("/orders/:orderId", authenticateToken, async (req, res) => {
  const orderId = req.params.orderId;
  ...
  const rowsQuery = `SELECT * FROM order_row WHERE OID = ? ORDER BY ROWID ASC`;
...
router.get("/orders", authenticateToken, async (req, res) => {
  ...
  const query = user.role === "admin"
    ? `SELECT o.OID, o.CID, o.USERID, o.ODATE, o.NOTES, u.username AS createdBy, c.CCOMPANY 
       FROM orders o 
       JOIN customer c ON o.CID = c.CID 
       JOIN users u ON o.USERID = u.id
       ORDER BY o.ODATE DESC`
    : `SELECT o.OID, o.CID, o.USERID, o.ODATE, o.NOTES, c.CCOMPANY 
       FROM orders o 
       JOIN customer c ON o.CID = c.CID 
       WHERE o.USERID = ?
       ORDER BY o.ODATE DESC`;
